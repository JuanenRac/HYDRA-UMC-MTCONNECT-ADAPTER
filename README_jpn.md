...
**正直な現状確認 - 今日実際に動くもの:** 単位変換(`src/units.ts`)、品質/タイムスタンプ/エラーコードのマッピング(`src/dataitem.ts`)、ポーリング/キャッシュ(`src/reader.ts`)、本物の HYDRA-UMC-SERVER に基づくマシンリーダー(`src/hydraServerReader.ts`)、そして HTTP サーバー(`src/server.ts`)はすべて本物であり、テスト済みである(6 ファイルにわたり 45 件のテストが成功しており、`supertest` を使った本物のリスニングソケットに対する本物の HTTP リクエストも含まれる——単なるコンパイルチェックではない)。デバイスモデルはちょうど 1 台の HydraNode(`hydra_umc_1`)にハードコードされている——これは HTTP インターフェースと XML の形状がエンドツーエンドで仕様に正しく準拠していることを証明しているが、複数台のロボットを監視する実際のデプロイでは HYDRA-UMC-SERVER 自身のライブなロボット一覧からこれを生成する必要があり、それはまだ行われていない。出力される XML は仕様に沿った形状(名前空間、`Device`/`DataItem` の id、共有される `instanceId`)であることが検証済みだが、実際のサードパーティ製 MTConnect Agent やコレクターに対してライブでテストされたことはまだない。これまでに何が実際に出荷されたかは `CHANGELOG.md` を参照。
...
* 📄 **XML データストリーム：** `GET /current` の実際にポーリングされる `<Events>`/`<Samples>` ブロックを標準の XML 形式で提供します。*(実装済み)*
* 🔌 **実際のマシンソース：** `HydraServerMachineReader`（`src/hydraServerReader.ts`、`HYDRA_UMC_SERVER_URL`）は実際に稼働している HYDRA-UMC-SERVER 自身の `GET /api/settings` をポーリングし、実際のロボット1台の実際の状態（`AVAILABLE`/`ACTIVE`/`INTERRUPTED`/`STOPPED`/`READY`）を報告します - `FixtureMachineReader` フォールバックだけではありません。詳細は `docs/API.md` を参照。*(実装済み)*
...
* **単位変換、品質分類、マシンの読み取りが3つの独立したモジュールである理由。** `src/units.ts`（純粋な変換演算）、`src/dataitem.ts`（品質/タイムスタンプ/エラーコードのマッピング）、`src/reader.ts`（`MachineReader` のポーリング/キャッシング）は、それぞれハードウェアや HTTP なしで単独でテスト可能です——これはまさに明示的な懸念事項です。完全な HTTP 往復を通じて発見される単位変換のバグは、切り分けに時間がかかり、見落としやすいのです。
