...
* 🔍 **エージェント互換性：** 既存の MTConnect エージェントおよびコレクターとシームレスに連携します。
...
* **コンパイルチェックだけでなく、実際の HTTP テスト。** `tests/server.test.ts` は `supertest`（実際にリッスンしているソケット上での実際の HTTP リクエスト）を使用して、`GET /probe` と `GET /current` が仕様どおりの形式の XML を返すことを検証します——一致する名前空間、両ドキュメント間で整合性のある `Device`/`DataItem` の id、そして共有された `instanceId`。
